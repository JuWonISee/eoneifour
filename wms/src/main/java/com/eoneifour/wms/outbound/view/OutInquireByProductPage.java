...
public class OutInquireByProductPage extends AbstractTablePage implements Refreshable {
	private MainFrame mainFrame;
	private StockProductDAO stockProductDAO;
	List<StockProduct> list;
	private String[] cols = { "ID", "제품명", "입고일", "출고위치" };

	private JTextField searchField;
	private JLabel keywordLabel;

	public OutInquireByProductPage(MainFrame mainFrame) {
		super(mainFrame);
	...
	public void initTopPanel() {
		...
		// ▶ 서쪽: 타이틀 추가
		JLabel title = new JLabel("제품명별 출고상태");
	...
	public void initTable() {
		list = stockProductDAO.selectAllOut(true);
	...
	private void performSearch() {
		...
		if (!keyword.isBlank()) {
			list = stockProductDAO.searchByConditionOut(keyword);
		} else {
			list = stockProductDAO.selectAllOut(true);
		}
